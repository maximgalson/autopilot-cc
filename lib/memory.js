// Autopilot Memory Layer v2.0.0
// JSON + tags + full-text search across all projects

const fs = require('fs');
const path = require('path');

const AUTOPILOT_HOME = process.env.AUTOPILOT_HOME || path.join(__dirname, '..');
const MEMORY_DIR = path.join(AUTOPILOT_HOME, 'memory');
...
const SESSIONS_DIR = path.join(AUTOPILOT_HOME, 'sessions');
...
function findRecurringTopic(summary, project) {
  ...
  for (const f of files) {
    try {
      ...
      for (const entry of log) {
        // Match by project name (strongest signal)
        ...
        if (overlap >= Math.min(2, Math.max(1, words.length * 0.3))) {
          matchCount++;
        }
      }
    } catch {}
  ...
  if (matchCount >= 3) return { count: matchCount };
  return null;
}
...
function getRelevantContext(activeTask, config) {
  ...
  // 3. Recent memories (by creation date, no access threshold)
  const recent = getAll()
